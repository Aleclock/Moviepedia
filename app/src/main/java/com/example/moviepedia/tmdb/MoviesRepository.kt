...
object MoviesRepository {
  ...
  fun getTVShowCredits(
    ...
    api.getTVShowCredits(tv_id = tvshow.id.toInt())
      .enqueue(object : Callback<GetMovieCreditsResponse> {
        override fun onResponse(call: Call<GetMovieCreditsResponse>, response: Response<GetMovieCreditsResponse>) {
          if (response.isSuccessful) {
            val responseBody = response.body()
            if (responseBody != null)
              onSuccess.invoke(responseBody)
  ...
  fun getSimilarTVShow(
      tvshow: TVShowTMDB,
      page: Int = 1,
      onSuccess: (tvShow: List<TVShowTMDB>) -> Unit,
      onError: () -> Unit
  ) {
    api.getSimilarTVShow(tv_id = tvshow.id.toInt(), page = page)
      .enqueue(object : Callback<GetTVShowResponse> {
        override fun onResponse(call: Call<GetTVShowResponse>, response: Response<GetTVShowResponse>) {
          if (response.isSuccessful) {
            val responseBody = response.body()
            if (responseBody != null)
